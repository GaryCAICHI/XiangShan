...
class Tage(implicit p: Parameters) extends BasePredictor with HasTageParameters with TopHelper with HalfAlignHelper {
  ...
  private val t0_useMeta   = t0_fire && t0_allHitBranchUseProvider && !t0_hasCondMispredict
  private val t0_readValid = t0_fire && !t0_useMeta

  private val t0_readBankConflict = t0_hasCond && t0_readValid && s0_fire && t0_bankIdx === s0_bankIdx
  ...
  tables.zipWithIndex.foreach { case (table, tableIdx) =>
    table.io.trainReadReq.valid         := t0_readValid
